...
namespace Iridescent.Utils.FluentEmail
{
    public class Email : IHideObjectMembers, IDisposable
    {
        ...
        public static SmtpSection DefaultSmtpSetting
        {
            get
            {
                System.Configuration.Configuration config =
                    ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                NetSectionGroup netSection = NetSectionGroup.GetSectionGroup(config);
                if (netSection != null)
                {
                    MailSettingsSectionGroup mailSettings = netSection.MailSettings;
        ...
        /// <summary>
        /// 从config中读取配置来创建Email对象
        /// </summary>
        /// <returns></returns>
        public static Email FromDefaultSetting()
        {
            SmtpSection defaultSetting = DefaultSmtpSetting;
            if(defaultSetting==null)
                throw new ConfigurationErrorsException("未配置smtp节点");

            Email email =
                From(defaultSetting.From).UsingClient(new SmtpClient(defaultSetting.Network.Host,
                                                                     defaultSetting.Network.Port)
                                                          {
                                                              Credentials =
                                                                  new NetworkCredential(
                                                                  defaultSetting.Network.UserName,
                                                                  defaultSetting.Network.Password),
                                                              EnableSsl = defaultSetting.Network.EnableSsl
                                                          });
            return email;
        }
        ...
        private void CheckRenderer()
        {
            ...
            {
                _renderer = new DefaultTemplateRender();
            }
        }
